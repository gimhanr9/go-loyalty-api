...
func (s *loyaltyService) EarnPoints(accountID string, description string, amount int) error {
	...
	resOrder, err := s.squareClient.Orders.Create(context.TODO(), reqOrder)
	if err != nil {
	...
	_, err = s.squareClient.Payments.Create(context.TODO(), reqPayment)
	if err != nil {
	...
	_, err = s.squareClient.Loyalty.Accounts.AccumulatePoints(context.TODO(), reqAccumulate)
	if err != nil {
...
func (s *loyaltyService) GetHistory(accountID string) ([]square.LoyaltyEvent, error) {
	...
	if resp == nil || resp.Events == nil {
		return nil, fmt.Errorf("no loyalty events found for account %s", accountID)
	}
